...
from routing_options import _report_narrative_summary  # noqa: E402
...
class ReportAgentTest(unittest.TestCase):
    ...
    def report(self, **changes) -> str:
        if changes.get("narrator") is not None:
            changes.setdefault("narrator_config", CONFIG)
    ...
    def test_a_broken_narrator_still_produces_the_report(self):
        ...
        with self.assertLogs(
            "hacklipse.adapters.reporting", level="WARNING",
        ) as captured:
        ...
        self.assertNotIn("narrator blew up", "\n".join(captured.output))

    def test_narrator_requires_format_v2(self):
        with self.assertRaises(ValueError):
            self.reporter(version="v1", narrator=LlmReportNarrator(
                llm_client=FakeLlm({}), config=CONFIG), narrator_config=CONFIG)

    def test_narrative_trace_is_a_separate_claim_without_body(self):
        facts = self.reporter().collect_facts(self.task)
        narrator = LlmReportNarrator(llm_client=FakeLlm(payload(facts)), config=CONFIG)
        ...
        claim = self.stores.evidence.get_many("run-1", result.new_evidence_ids)[0]
        self.assertEqual(claim.created_by, "llm_report_narrator")
        ...
        self.assertNotIn("run_summary", claim.observation)
        self.assertNotIn("finding_summaries", claim.observation)
        self.assertTrue(claim.observation["input_fingerprint"])
        run = self.stores.runs.get("run-1").with_updates(
            evidence_ids=result.new_evidence_ids,
        )
        self.stores.runs.save(run)
        summary = _report_narrative_summary(SimpleNamespace(stores=self.stores), run)
        self.assertEqual(summary["claim_count"], 1)
        self.assertEqual(summary["llm_calls"], 1)
        self.assertEqual(summary["selection_source_counts"], {"llm": 1})
